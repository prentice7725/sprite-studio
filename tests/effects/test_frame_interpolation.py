...
from sprite_studio.effects.interpolate import aligned_pair_on_chroma, interpolate_between
...
def test_unknown_provider_rejected(tmp_path: Path) -> None:
    from sprite_studio.effects.interpolate import gen_interpolator
...
def test_tween_scale_normalized_to_reference_pair(tmp_path: Path) -> None:
    ...
    from sprite_studio.effects.interpolate import normalize_tween_scale, _chroma_content_bbox
    chroma = MAGENTA
...
def test_interpolate_normalizes_stub_output(tmp_path: Path) -> None:
    ...
    from sprite_studio.effects.interpolate import _chroma_content_bbox
    run = _build_run(tmp_path)
