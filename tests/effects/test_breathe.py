...
from sprite_studio.effects.anatomy import analyze
from pathlib import Path

from sprite_studio.effects.breathe import (DEFAULT_DEPTH, MAX_ROW_STRAIN, SMOOTH_CYCLE_FRAMES, TAPER,
...
from sprite_studio.curate.curation import state_breathe
from sprite_studio.frames.extract import solid_alpha_bbox
...
def test_thinning_normalizes_warp_doubled_outline(monkeypatch) -> None:
    ...
    import sprite_studio.effects.breathe as breathe_mod
...
def test_out_of_range_values_are_refused_instead_of_clamped() -> None:
    ...
    계약이 어긋난다 — 한쪽으로 정했다."""
    from sprite_studio.curate.curation import (BREATHE_BREATHS_MAX, BREATHE_DEPTH_MAX,
...
def test_the_curator_ui_cap_matches_the_schema_bound() -> None:
    ...
    from sprite_studio.curate.curation import BREATHE_BREATHS_MAX, BREATHE_DEPTH_MAX, BREATHE_LAG_MAX
    import re
...
def test_manual_band_anchors_protection_to_the_band() -> None:
    ...
    from sprite_studio.effects.breathe import protect
    src = _humanoid()
    auto = analyze(src)
