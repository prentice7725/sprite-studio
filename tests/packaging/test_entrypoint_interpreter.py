...
`sprite_studio.frames.extract` at module load).
...
# all: a path anchored outside the current directory (`$SPRITE_STUDIO_ROOT/...`,
# the `$SG` shorthand docs/curation.md defines, the elliptical `.../<script>.py`
# form), a repo-relative `scripts/<script>.py`, or `-m sprite_studio...`. These
...
_AGENT_REGISTER_BARE = re.compile(
    _BARE_INTERPRETER
    + r"(?:"
    + r"[\"']?(?:\$\{?(?:SPRITE_STUDIO_ROOT|SG)\b|\.\.\./)"   # skill-absolute path
    + r"|-m\s+sprite_studio"                                       # or the package CLI
...
def test_agent_facing_docs_never_invoke_a_global_python() -> None:
    ...
    assert not offenders, (
        ...
        "($SPRITE_STUDIO_ROOT/.venv/bin/python), not a global python3:\n"
        + "\n".join(offenders)
    )
