...
installed. Those routes go through `-m sprite_studio.<module>`, and the last test holds them
to it.
"""
...
from sprite_studio.serve.serve_curation import CURATOR_DIR

ROOT = Path(__file__).resolve().parent.parent.parent
PACKAGE_DIR = ROOT / "sprite_studio"
...
def _declared_package_data() -> list[str]:
    """The `sprite_studio = [...]` patterns under `[tool.setuptools.package-data]`.
    ...
    entry = re.search(r"(?ms)^sprite_studio\s*=\s*\[(.*?)\]", section.group(1))
    assert entry, "[tool.setuptools.package-data] declares nothing for sprite_studio"
...
def test_the_package_data_patterns_collect_every_curator_asset() -> None:
    ...
    assert not missing, (
        ...
        f"404 for anyone who installed sprite-studio instead of cloning it")
...
def test_the_built_wheel_carries_every_curator_asset(wheel_names: set[str]) -> None:
    ...
    on_disk = {f"sprite_studio/serve/curator/{p.relative_to(CURATOR_DIR).as_posix()}"
               for p in _curator_assets()}
    in_wheel = {n for n in wheel_names if n.startswith("sprite_studio/serve/curator/")}

    assert not sorted(on_disk - in_wheel), (
        f"the built wheel is missing {sorted(on_disk - in_wheel)} — the SPA loads them from "
        f"an installed sprite_studio, so an install serves a shell whose scripts 404")
...
def test_the_built_wheel_carries_every_package_module(wheel_names: set[str]) -> None:
    ...
    A hand-listed `packages = ["sprite_studio"]` once dropped all of `sprite_studio/gen/`, and
    `sprite-studio --help` (cli.py imports `gen` at module scope) died on every non-editable
...
def test_the_package_never_reaches_into_the_scripts_wrappers(module: str) -> None:
    ...
    assert not offenders, (
        ...
        f"`-m sprite_studio.<module>` instead.")
