...
lands. The runtime ones hold whatever `sprite_studio/frames/extract.py` does, because the
...
def _entry_scripts() -> list[Path]:
    ...
    Derived rather than listed: a script that stops importing `sprite_studio` also
    stops needing NumPy, and one that starts importing it joins this population
    without anyone remembering to add it here.
    """
    return sorted(p for p in (ROOT / "scripts").glob("*.py") if _imports_at_load(p, "sprite_studio"))


# The registers SKILL.md documents, plus a bare package import for downstream importers.
COMMANDS = {"package import": ["-c", "import sprite_studio"], "cli module": ["-m", "sprite_studio.cli", "--help"]}
...
def test_numpy_is_imported_in_exactly_one_shipped_module() -> None:
    ...
    assert importers == [GATE], (
        f"shipped code must reach NumPy through {GATE} (`from sprite_studio._deps import np`), not "
...
def test_the_gate_hands_back_the_real_numpy() -> None:
    ...
    from sprite_studio import _deps
