...
`python -m sprite_studio.serve.serve_curation` and the `scripts/serve_curation.py` wrapper. A copied
...
def test_every_launch_form_exposes_the_same_flags() -> None:
    subcommand = _help_options("-m", "sprite_studio.cli", "curation")
    module = _help_options("-m", "sprite_studio.serve.serve_curation")
