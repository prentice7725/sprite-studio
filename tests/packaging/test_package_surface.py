...
def test_mcp_import_surface_modules_expose_callable_run(module_name: str) -> None:
    module_path = "sprite_studio.gen" if module_name == "gen" else qualified(module_name)
...
def test_cli_parser_helpers_exist(helper_name: str) -> None:
    cli = importlib.import_module("sprite_studio.cli")

    assert callable(getattr(cli, helper_name, None)), f"sprite_studio.cli.{helper_name} must be callable"
...
def test_package_modules_import_siblings_by_package_path() -> None:
    """`from sprite_studio.curate.curation import …`, never `from curation import …`.

    A bare sibling import only resolves when `scripts/` happens to be on `sys.path`, so it
    survives `python scripts/<tool>.py` and dies under `python -m sprite_studio.<tool>` or any
...
def test_installed_distribution_version_matches_skill_frontmatter() -> None:
    try:
        installed_version = metadata.version("sprite-studio")
    except metadata.PackageNotFoundError:
        pytest.skip("sprite-studio distribution metadata is only available after editable/package install")
