...
from sprite_studio.curate.curation import frame_variant, run_revision
...
def test_frame_variant_resolution_order() -> None:
    ...
    repaired = {"states": {"idle": {"source_variant": "repaired", "source_variant_revision": "abc123"}}}
    assert frame_variant(repaired, "idle") == "repaired"
    with pytest.raises(SystemExit, match="without a source_variant_revision"):
        frame_variant({"states": {"idle": {"source_variant": "repaired"}}}, "idle")
