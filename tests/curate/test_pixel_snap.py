...
import sprite_studio.frames.extract as extract_module
from sprite_studio.curate.curation import apply_transform, pixel_snap_scale
...
def test_twins_share_pixel_perfect_footprint(tmp_path) -> None:
    ...
    for index, grid in enumerate(grids):
        ...
        if row.get("orig_files"):
            ...
            # 어긋남은 0.0 논리픽셀, plan sprite-studio/grid-record-exactness). 0 으로 두지 않는
...
def test_partial_generation_view_tolerance(tmp_path) -> None:
    ...
    from sprite_studio.frames.extract import load_consistent_frames_manifest
    run_dir = _build_pp_run(tmp_path)
    request = _json.loads((run_dir / "sprite-request.json").read_text(encoding="utf-8"))
...
def test_apply_pixel_edits_sidecar() -> None:
    ...
    from sprite_studio.curate.curation import apply_pixel_edits
