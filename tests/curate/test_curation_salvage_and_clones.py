...
from sprite_studio.curate.curation import (
    load_curation,
...
def _write_stamped(run: Path, states: dict) -> dict:
    payload = stamp_curation(run, {"version": 1, "kind": "sprite-studio-curation", "states": states})
...
def test_legacy_unstamped_sidecar_drops_all_and_the_save_backs_it_up(fixture_run_dir: Path) -> None:
    run = fixture_run_dir
    _extract(run)
    legacy = {"version": 1, "kind": "sprite-studio-curation",
    ...
    backup = write_curation_atomic(run, {"version": 1, "kind": "sprite-studio-curation", "states": {}})
