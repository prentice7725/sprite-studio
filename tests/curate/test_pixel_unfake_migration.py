...
from sprite_studio.curate.curation import (CURATION_FILENAME, frame_variant, load_curation_report,
                                 run_revision)
from sprite_studio.spec.runio import load_request, migrate_request_file
...
def test_unrelated_edits_do_not_migrate_the_schema(tmp_path) -> None:
    ...
    from sprite_studio.spec.runio import write_request
...
def test_no_production_path_reads_the_request_outside_the_gate() -> None:
    ...
    **AST 로 본다.** 첫 버전은 한 줄 정규식이었고, 실제 회귀 형태(경로를 변수에 담고 다음 줄에서
    ...
    offenders: list[str] = []
    for path in sorted(list((root / "sprite_studio").glob("*.py")) + list((root / "scripts").glob("*.py"))):
