...
읽고 나서 `.sprite-studio.lock` (파이프라인 단계용 단일 writer 락)을 잡았다. 두 가지가 동시에
...
from sprite_studio.effects import interpolate, reroll
from sprite_studio.spec import runio
from sprite_studio.spec.runio import REQUEST_FILENAME, migrate_request_file
...
def _write_request(run_dir: Path, fit: dict) -> None:
    ...
    (run_dir / REQUEST_FILENAME).write_text(json.dumps({
        "version": 1, "kind": "sprite-studio-request", "engine": "component-row",
...
def test_migration_does_not_take_the_pipeline_writer_lock(tmp_path) -> None:
    """이관은 `.sprite-studio.lock` 도메인을 쓰지 않는다 — 그 락은 request 를 지키지 않는다.
...
def test_every_request_write_is_inside_the_publish_guard() -> None:
    ...
    **만드는** 경로(`prepare`, `unpack_atlas` 의 스테이징)는 제외한다 — 기존 문서를 읽어
    ...
    offenders: list[str] = []
    for path in sorted((root / "sprite_studio").rglob("*.py")):
    ...
    runio_path = root / "sprite_studio" / "spec" / "runio.py"
    ...
    for node in ast.walk(runio_tree):
        ...
        for child in ast.walk(node):
            if (isinstance(child, ast.Call) and _callee_name(child.func) == "atomic_write_text"
                ...
                offenders.append(f"sprite_studio/spec/runio.py:{child.lineno} "
