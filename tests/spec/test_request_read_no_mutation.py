...
(`runio.migrate_request_file` / `sprite-studio migrate-request`) 에서만 원자적으로 일어난다.
"""
...
from sprite_studio.curate import curation as curation_mod
from sprite_studio.curate.curation import state_revision
from sprite_studio.spec.runio import REQUEST_FILENAME, load_request
...
def read_everything(run_dir: Path) -> None:
    ...
    from sprite_studio.curate.curation import load_curation, load_curation_report
...
def test_read_api_surface_is_byte_stable_when_the_sidecar_is_stale(synthetic_fixture_a_run: Path) -> None:
    ...
    from sprite_studio.curate.curation import CURATION_FILENAME
...
def test_loader_holds_no_writer_call(synthetic_fixture_a_run: Path) -> None:
    ...
    **존재하지 않는다**는 것을 AST 로 본다."""
    ...
    import sprite_studio.spec.runio as _runio_mod
