...
4. the published CLI (`sprite-studio compose-layers`, §7) — exit code, summary, and
...
from sprite_studio.curate.curation import empty_curation, write_curation_atomic
...
def test_load_report_distinguishes_absent_from_foreign(tmp_path: Path) -> None:
    ...
    path.write_text(json.dumps({"kind": "sprite-studio-recolor-report"}), encoding="utf-8")
    with pytest.raises(SystemExit):
