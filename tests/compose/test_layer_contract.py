...
1. `sprite_studio.compose.layers` — the request-level contract (profiles, tracks, stacks).
...
from sprite_studio.compose import layers
...
def test_layer_data_never_enters_the_curation_sidecar() -> None:
    ...
    schema_doc = (ROOT / "sprite_studio" / "curate" / "curation.py").read_text(encoding="utf-8")
...
def test_layer_doc_declares_the_whole_vocabulary() -> None:
    ...
    for key in ("rig.landmarks", "layers", "allow_clip", "full_body_override"):
        assert key in text
    assert "sprite_studio/compose/layers.py" in text
...
def test_the_bake_command_is_documented_where_an_agent_looks() -> None:
    ...
    assert "sprite-studio compose-layers" in skill
    assert "scripts/compose_layers.py" in skill, "the script map's required_scripts list"
    for clause in ("sprite-studio compose-layers", "--names", "sprite_studio/compose/compose_layers.py"):
