...
from sprite_studio.serve.serve_curation import CurationHandler, _url, build_run_state  # noqa: E402
from sprite_studio.frames.unpack_atlas import import_png_groups  # noqa: E402
...
def _snapshot(run: Path) -> dict:
    return {p.relative_to(run): p.read_bytes()
            for p in run.rglob("*") if p.is_file() and p.name != ".sprite-studio.lock"}
...
def test_publish_phase_failure_rolls_back(tmp_path, monkeypatch):
    ...
    from sprite_studio.frames import unpack_atlas as ua
...
def test_publish_never_removes_out_dir(tmp_path, monkeypatch):
    ...
    out = tmp_path / "run"
    assert _run_import(pngs, out, "--force").returncode == 0
    from sprite_studio.spec.runio import LOCK_FILENAME
    _png(pngs / "items" / "2-b.png")
...
def test_api_run_reader_isolated_from_concurrent_publish(tmp_path):
    ...
    from sprite_studio.spec import runio
...
@pytest.mark.parametrize("modname,kwargs", [
    ("sprite_studio.qa.preview", {}),
    ("sprite_studio.compose.compose_gif", {}),
    ("sprite_studio.compose.compose_cycle", {"state": "walk", "name": "cyc"}),
])
...
def test_inspect_reader_isolated_from_concurrent_commit(fixture_run_dir):
    ...
    from conftest import run_script
    from sprite_studio.spec import runio
    from sprite_studio.qa.inspect import inspect_run
...
def test_stale_curation_post_rejected_across_run_generations(tmp_path):
    ...
    from sprite_studio.serve.serve_curation import CurationHandler
    ...
    def post(revision):
        body = _json.dumps({"version": 1, "kind": "sprite-studio-curation", "runRevision": revision,
...
def test_stored_curation_ignored_after_frame_regeneration(tmp_path):
    ...
    from sprite_studio.serve.serve_curation import build_run_state, write_curation_atomic
    from sprite_studio.curate import curation as cur
    ...
    write_curation_atomic(out, {"version": 1, "kind": "sprite-studio-curation",
...
def test_unstamped_curation_ignored(tmp_path):
    ...
    (out / "curation.json").write_text(_json.dumps({
        "version": 1, "kind": "sprite-studio-curation",
        "states": {"items": {"selected": [1], "transforms": {"1": {"dx": 9}}}}}))
    ...
    from sprite_studio.serve.serve_curation import write_curation_atomic
    write_curation_atomic(out, {"version": 1, "kind": "sprite-studio-curation",
...
def test_extract_frame_publish_holds_publish_guard(tmp_path):
    ...
    from sprite_studio.spec import runio
    from sprite_studio.frames.extract import _commit_generation
...
def test_autosave_preserves_pixel_perfect_the_view_does_not_author(tmp_path, field, where):
    ...
    **굽기가 읽는 변종이 바뀐다**(`frame_variant`). `.plain.png` 가 없는 줄이 `plain` 으로
    ...
    것과 같은 상태였다."""
    from sprite_studio.curate.curation import frame_variant, stamp_curation
    ...
    curation = {"version": 1, "kind": "sprite-studio-curation", "states": {"items": {}}}
    if where == "top":
    ...
    try:
        ...
        body = json.dumps({"version": 1, "kind": "sprite-studio-curation", "runRevision": revision,
