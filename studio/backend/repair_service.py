...
def _load_refine_residuals(
    run_dir: Path, request: dict[str, Any], state: str, source_files: list[Path],
) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    """Refine's residual handoff, gated on the refined output's *content*
    revision rather than its filenames (§3): `frame-0.png` regenerated by a
    later refine is still named `frame-0.png`, so a filename-only check would
    hand Repair residuals that describe pixels which no longer exist.

    Returns ``(residuals, provenance)``. ``residuals`` is `[]` whenever they
    are not safe to use — but the *reason* is always in ``provenance``
    (never a silent empty list): "none" (no report / no residuals to begin
    with), "stale" (content changed since the report was written, or the
    report predates revision tracking and can't be trusted), or "accepted".
    """
    report_path = _state_dir(run_dir, request, state) / "refined" / "refine.report.json"
    current_revision = content_revision(source_files)
    if not report_path.is_file():
        return [], {"residual_status": "none"}
    try:
        report = json.loads(report_path.read_text(encoding="utf-8"))
    except (json.JSONDecodeError, OSError) as exc:
        return [], {"residual_status": "none", "reason": f"refine.report.json unreadable: {exc}"}
    if report.get("kind") != "asset-studio-sprite-refine":
        return [], {"residual_status": "none"}
    residuals = list(report.get("residuals") or [])
    if not residuals:
        return [], {"residual_status": "none"}
    expected_revision = report.get("output_revision")
    if not isinstance(expected_revision, str):
        # A refine report written before revision tracking existed. Its
        # residuals cannot be vouched for by content, and filenames alone
        # are exactly the false confidence this fix removes — report it
        # plainly instead of quietly falling back to the old filename check.
        return [], {
            "residual_status": "stale",
            "reason": "refine report predates output_revision tracking",
...
def analyze_state(run_dir: Path, state: str) -> dict[str, Any]:
    ...
    residuals, residual_provenance = _load_refine_residuals(run_dir, request, state, source_files)
    ...
    payload = {
        ...
        "state": state,
        "source": "refined",
        "source_revision": content_revision(source_files),
...
def repair_state(run_dir: Path, state: str, *, candidate_ids: set[str] | None = None) -> dict[str, Any]:
    ...
    payload = {
        ...
        "output_revision": content_revision(output_paths),
        "logical_size": list(frames[0].size),
        "working_size": list(source_size),
...
def repaired_files(run_dir: Path, state: str) -> list[Path]:
    ...
    try:
        ...
        return []
    if log.get("source_revision") != content_revision(source_files):
        return []
    ...
    if not outputs or not all(path.is_file() for path in outputs):
        return []
    if log.get("output_revision") != content_revision(outputs):
        return []
    return outputs
...
def apply_ai_micro_fix(run_dir: Path, state: str, job_id: str, result_path: Path) -> dict[str, Any]:
    ...
    frames, source_files, source_size = _load_logical(run_dir, request, state)
    if job.get("source_revision") != content_revision(source_files):
...
def review_data(run_dir: Path, state: str) -> dict[str, Any]:
    ...
    if payload is not None:
        try:
            if payload.get("source_revision") != content_revision(_refined_files(run_dir, request, state)):
                payload = None
        except FileNotFoundError:
            payload = None
