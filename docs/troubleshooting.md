...
- **대처**: 에이전트 쓰기는 반드시 정식 경로로 —
  `sprite_studio.curate.curation.load_curation(run_dir)` 로 읽고 수정한 뒤
...
- **증상**: `sprite-studio gen` 자식(provider CLI)이 출력 없이 멈춘다. 인증도 정상.
...
## Codex CLI가 이미지를 만들지 못함

- **원인**: `codex` 실행 파일이 설치되고 로그인되어 있다는 사실만으로는 현재
  Codex 세션에 `image_gen` 도구가 제공된다는 뜻이 아니다. 일반적인 Codex CLI는
  텍스트 작업만 제공할 수 있다.
- **대처**: Studio의 `GET /api/providers` 결과에서 Codex가 비활성으로 표시되면
  Grok을 선택한다. 이미지 생성이 제공되는 Codex 배포판임을 별도로 확인한 경우에만
  `SPRITE_STUDIO_CODEX_IMAGE_GEN=1`을 설정한다. 확인되지 않은 환경에서 이 값을
  켜면 Codex child가 다시 장시간 대기할 수 있다.

provider가 실제 생성 중 멈추면 API는 `504 image provider timeout`으로 응답한다.
이는 잘못된 요청의 `500`과 구분되며, provider 세션을 확인한 뒤 재시도한다.
...
- **원인**: provider 가 레이아웃 가이드의 가로 슬롯 배치를 무시함 (grok 로코모션
  행에서 관측).
- **대처**: 해당 행만 재생성 — `/api/providers`에서 이미지 생성 capability가
  확인된 provider를 선택하고 프롬프트의 슬롯/가로폭 계약을 강조한다. 현재 Codex
  CLI가 비활성이라면 Grok 또는 별도로 검증된 이미지 provider를 사용한다. 로컬에서
  자르거나 회전시켜 살리지 않는다
  (필수 게이트: raw 수리 금지).
