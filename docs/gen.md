# `sprite-studio gen` — provider-backed image generation (engine SSoT)

Generation is a first-class engine module (`sprite_studio/gen/`), not an external
...
1. **`SPRITE_STUDIO_DEFAULT_PROVIDER`** env var (`codex` or `grok`) — the user override.
...
grok default (`SPRITE_STUDIO_DEFAULT_PROVIDER=grok`) has no reverse fallback — a down
...
backend), `provider_resolved_from` (`explicit` / `SPRITE_STUDIO_DEFAULT_PROVIDER` /
...
```bash
sprite-studio gen \
  [--provider codex|grok] # optional; default = SPRITE_STUDIO_DEFAULT_PROVIDER env → codex (observable grok fallback if codex is down)
...
Backward-compatible wrapper: `$SPRITE_STUDIO_ROOT/.venv/bin/python $SPRITE_STUDIO_ROOT/scripts/generate_sprite_image.py …` (same args).
...
- **`--transparent`**: the raw is keyed to clean RGBA via the ported transparent
  contract (`sprite_studio/gen/chroma.py`). Generate on a solid `#FF00FF` (or `#00FF00`)
...
- `--report` writes a `sprite-studio-image-report` JSON: provider, prompt, out/raw paths,
...
The correction loop (`sprite-studio correction-loop --provider-command …`) can drive this
