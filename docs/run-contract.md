...
> three things every sprite-studio run must satisfy so that **any agent who serves a
...
| Generate | `sprite-studio gen` (`generate_sprite_image.py`) | `prompts/<state>.txt` + refs | verified `raw/<state>.png` strip + audit raw/report |
...
| Curate (opt) | `sprite-studio curation` (`serve_curation.py`) + `curation.py` | `frames/` | `curation.json` sidecar |
| Compose | `compose_sprite_atlas.py` | `frames/` + `curation.json` | `sprite-sheet-alpha.png`, `manifest.json`, `*.report.json` |
| Engine export (opt) | `sprite-studio export-aseprite` (`sprite_studio/compose/export_aseprite.py`) | composed `manifest.json` + existing atlas | `exports/aseprite.json`, or `exports/aseprite/<state>.json` for Flame |
| Recolor (opt) | `sprite-studio recolor` / `recolor-palette` (`sprite_studio/effects/recolor.py`) | base sheet (default `sprite-sheet-alpha.png`) + recolor spec | `variants/<name>.png`, optional `variants/<name>.manifest.json`, `variants/recolor.report.json` |
| Layer bake (opt) | `sprite-studio compose-layers` (`sprite_studio/compose/compose_layers.py`) | `frames/` + `curation.json` + the request's `rig` / `layers` | `layers/<name>.png`, `layers/<name>.manifest.json`, `layers/layers.report.json` (published as one set) |
...
components, pixel-unfake path, the `.sprite-studio.lock` single-writer rule, the
...
<target>/assets/generated/sprites/<character-id>/
  ...
  references/anchors/<dir>-anchor-x8.png # DERIVED CACHE: the curated direction-anchor frame, baked
                                         #   ×8 NEAREST for row generation. `sprite-studio anchor`
                                         #   (SSoT sprite_studio/curate/anchor.py) rewrites it on demand —
  ...
  # <direction>_<pose>, 파일 경로만 분리. 리졸버 SSoT = sprite_studio/spec/layout.py.
  ...
  variants/                          # only when sprite-studio recolor runs — baked colourway sheets
  ...
  .sprite-studio.lock                   # single-writer lock (runio.py); a live holder blocks a second writer (§7 guarantee boundary)
...
schema writer (`runio.migrate_request_file`, exposed as `sprite-studio migrate-request
...
- **Schema migration is an explicit command.** It is a dry run unless `--apply` is passed.
  ...
  file atomically. It deliberately does not take the pipeline `.sprite-studio.lock`: that lock
...
The webview (`sprite_studio/serve/curator/*`) renders exactly four contract elements from that
...
- **Cross-process single-writer.** `.sprite-studio.lock` blocks a second **process** from writing
...
- **Thread-level write isolation.** `.sprite-studio.lock` re-entry is keyed per process, not per
...
cases above and do not contradict this. If sprite-studio is ever re-homed as a multi-user or server
