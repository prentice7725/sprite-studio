# Directional / 45° Anchor Workflow — sprite-studio reference
...
2. **Direction gate** — create direction anchors before action rows:
   ...
   **The anchor image is the CURATED frame, and one command bakes it.**
   ...
   ```bash
   sprite-studio anchor --run-dir <run> --direction <dir>      # writes references/anchors/<dir>-anchor-x8.png
   sprite-studio anchor --run-dir <run> --for-state <state>    # prints the ref path to attach for that row
   ```
   ...
   **Which frame is the anchor** (`sprite_studio/curate/anchor.py` owns the rule):
   ...
   - override: the frame the human pinned in the curation view (pin button on the
     frame card) or via `sprite-studio anchor --pick <state>#<index>`. Any instance
