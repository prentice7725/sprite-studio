...
`sprite-studio export-aseprite` describes a composed sprite-studio atlas using the JSON schema consumed by Aseprite loaders. It does not create an editable `.aseprite` source file and it does not re-encode the PNG.

Run it after `compose-atlas`:

```bash
$SPRITE_STUDIO_ROOT/.venv/bin/sprite-studio export-aseprite --run-dir <run-dir>
```
...
| sprite-studio manifest | Aseprite-compatible JSON |
|---|---|
...
Repeated playback instances keep repeated rectangles under distinct numeric frame keys. This preserves sprite-studio's shared-cell representation while presenting the sequential frame keys expected by engine loaders.
...
```bash
$SPRITE_STUDIO_ROOT/.venv/bin/sprite-studio export-aseprite --run-dir <run-dir> --format json-hash --split-states
```
...
- `loop` remains in `manifest.json`. Aseprite frame tags carry direction and range but no sprite-studio loop policy, and Flame's Aseprite factory constructs its default looping animation without reading tags.
