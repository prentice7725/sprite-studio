...
`slice_sheet_cells.py` (= `sprite_studio.cli slice-sheet`) turns ONE generated
...
Alpha truth is `sprite_studio.frames.extract.remove_chroma_background` — the same
...
```bash
$SPRITE_STUDIO_ROOT/.venv/bin/python $SPRITE_STUDIO_ROOT/scripts/slice_sheet_cells.py \
