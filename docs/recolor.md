# Recolor — deterministic palette-swap bake + colourway pick — sprite-studio reference
...
> `sprite_studio/effects/recolor.py`. View surface: `sprite_studio/serve/curator/src/recolor.js` +
...
Draft kind: `"kind": "sprite-studio-palette"`. This is authoring scaffolding, not a
...
```bash
$SPRITE_STUDIO_ROOT/.venv/bin/python -m sprite_studio.cli recolor ...
$SPRITE_STUDIO_ROOT/.venv/bin/python $SPRITE_STUDIO_ROOT/scripts/recolor.py ...
```
...
- `kind` must be `sprite-studio-recolor` — anything else fails loud.
- `match`:
...
`recolor.report.json` (`kind: "sprite-studio-recolor-report"`) is the one truth for
...
pixels. Reader: `sprite_studio.effects.recolor.load_report(run_dir)`.
...
```bash
$SPRITE_STUDIO_ROOT/.venv/bin/sprite-studio curation --run-dir <run-dir>
```
...
$SPRITE_STUDIO_ROOT/.venv/bin/python \
  $SPRITE_STUDIO_ROOT/scripts/compose_sprite_atlas.py --run-dir <run>
...
#    Save as e.g. <run>/recolor.spec.json with kind "sprite-studio-recolor".
...
$SPRITE_STUDIO_ROOT/.venv/bin/sprite-studio curation --run-dir <run>
```
