...
flowchart TD
    ...
    GUIDES --> GEN["sprite-studio gen<br/>(codex or grok; one image per state — the only AI step)"]
...
lock file `.sprite-studio.lock` taken by the extract/compose/export/unpack
...
- **Which image is "the accepted anchor" is code, not judgment** (`sprite_studio/curate/anchor.py`,
  `sprite-studio anchor`): it resolves the human's pin (`curation.json` `anchors.<direction>`)
...
1. **Pitch detection** — `detect_pixel_pitch()` scores each candidate pitch by
   ...
   own detected pitch** (maintainer 2026-07-20, plan `sprite-studio/per-frame-pixel-grid`:
...
2. **Grid-phase snap** — the phase is **measured, not inferred from the pitch
   ...
   (plan `sprite-studio/frame-pitch-consensus-eats-a-row`; see
...
```json
{ "version": 1, "kind": "sprite-studio-curation", "run_revision": "9f3c1a0b7e2d4c58",
  "pixel_unfake": true,
...
- **rig runs only** (`sprite_studio/compose/layers.py`, `compose_layers.py`): `rig.profile` +
...
`runio.py`'s `.sprite-studio.lock` (§2 above).
...
`sprite-studio` is inspired by the Apache-2.0 `hatch-pet` skill but ships no pet
