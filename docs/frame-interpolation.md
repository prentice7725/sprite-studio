...
`sprite_studio/effects/interpolate.py` (`scripts/interpolate_frames.py`) makes an **in-between
...
The backend is the engine's own generation layer (`sprite_studio.gen`): the two aligned
...
```bash
$SPRITE_STUDIO_ROOT/.venv/bin/python $SPRITE_STUDIO_ROOT/scripts/interpolate_frames.py \
...
4. `tween_prompt` composes the prompt deterministically from request truth (character
   description, chroma color, t) and `sprite_studio.gen.generate_image` runs the provider
