# Pixel Unfake Fit (`fit` / `pixel_unfake`) — sprite-studio reference
...
`sprite-studio/frame-pitch-consensus-eats-a-row`).
...
`sprite_studio/serve/serve_curation.py`)는 여전히 `detect_pixel_grid` 가 돌려주는 히스토그램 위상으로
절단선을 만든다. 그 절단선은 **표시용 오버레이 선 그리기에 그치지 않는다** — 베이스 편집기가
각 블록의 중심 픽셀을 raw 에서 샘플해 논리 편집 캔버스를 만들고(`sprite_studio/serve/curator/src/base-editor.js`),
...
**위상 출처를 바꿨을 때의 출력 변화** (plan `sprite-studio/frame-pitch-consensus-eats-a-row`):
...
줄었다 (plan `sprite-studio/best-phase-hotspot`: 채점을 정수 정확 산술로 재정식화해 채점 코어
...
- **디스크 이관** — 사용자가 명시적으로 부르는 단일 writer 에서만 한다:
  ...
  ```bash
  $SPRITE_STUDIO_ROOT/.venv/bin/sprite-studio migrate-request <run-dir>          # dry run
  $SPRITE_STUDIO_ROOT/.venv/bin/sprite-studio migrate-request <run-dir> --apply  # 실제 쓰기
  ```
