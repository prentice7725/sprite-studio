...
>
> Declaration validation is implemented by [`../sprite_studio/compose/layers.py`](../sprite_studio/compose/layers.py)
> and pinned by `tests/test_layer_contract.py`; the composite bake is
> [`../sprite_studio/compose/compose_layers.py`](../sprite_studio/compose/compose_layers.py), pinned by
...
| `curation.json` | the human's edits | webview POST and `sprite-studio anchor --pick`, both via `curation.write_curation_atomic` | `run_revision` + per-state `revision` | **nothing** |
...
- **B2 — there is no central request schema.** Every stage reads the keys it wants
  ...
  (`sprite_studio/compose/layers.py`) that every layer-aware entry point calls, instead of
...
The composer ([`../sprite_studio/compose/compose_layers.py`](../sprite_studio/compose/compose_layers.py),
...
provenance record for **every** composite (`kind: "sprite-studio-layers-report"`), not
...
`sprite_studio.compose.layers.validate_layer_request(request)` returns **every** violation, in a
...
The `sprite-studio compose-layers` subcommand, the `-m sprite_studio.compose.compose_layers` module
