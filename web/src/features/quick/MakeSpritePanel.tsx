...
import type { QuickDither, QuickFrameCount, QuickLogicalHeight, QuickMotion, QuickOutline, QuickPalette, QuickSession } from '../../api'
...
export default function MakeSpritePanel({ session, busy, onSubmit, onBack }: Props) {
  ...
  const [pixelSize, setPixelSize] = useState<QuickLogicalHeight>(128)
  ...
  return <section className="panel quick-make-panel" aria-labelledby="quick-make-heading">
    ...
    <label className="check-row"><input type="checkbox" checked={pixelize} onChange={(event) => setPixelize(event.target.checked)} /> {text.pixelizeBefore}<span className="check-detail">{pixelSize}px · {palette === 'auto' ? text.auto : `${palette} ${text.colors}`}</span></label>
    {pixelize && <details className="advanced-options" open><summary>{text.pixelizeSettings}</summary><div className="form-row"><label>{text.logicalHeight}<select value={pixelSize} onChange={(event) => setPixelSize(Number(event.target.value) as QuickLogicalHeight)}>{LOGICAL_HEIGHT_OPTIONS.map((value) => <option key={value} value={value}>{value}</option>)}</select></label><label>{text.palette}<select value={palette} onChange={(event) => setPalette(event.target.value === 'auto' ? 'auto' : Number(event.target.value) as QuickPalette)}><option value="auto">{text.auto}</option><option value={16}>16</option><option value={24}>24</option><option value={32}>32</option><option value={48}>48</option></select></label></div><div className="form-row"><label>{text.dither}<select value={dither} onChange={(event) => setDither(event.target.value as QuickDither)}><option value="none">{text.none}</option><option value="ordered-low">{text.low}</option><option value="ordered">{text.ordered}</option></select></label><label>{text.outline}<select value={outline} onChange={(event) => setOutline(event.target.value as QuickOutline)}><option value="preserve">{text.preserve}</option><option value="auto">{text.auto}</option></select></label></div><label className="check-row"><input type="checkbox" checked={backgroundCleanup} onChange={(event) => setBackgroundCleanup(event.target.checked)} /> {text.cleanup}</label></details>}
    ...
    <div className="button-row"><button className="primary-button" type="button" disabled={busy} onClick={() => onSubmit({ motion, custom_motion: customMotion, directions, frames, pixelize, pixel_size: pixelSize, palette, dither, outline, background_cleanup: backgroundCleanup, sprite_source: pixelize ? 'pixelized' : session.source_selection, generation_strategy: 'AUTO' })}>{busy ? text.starting : text.makeSprite}</button><button className="secondary-button" type="button" onClick={onBack}>{text.back}</button></div>
  </section>
}
