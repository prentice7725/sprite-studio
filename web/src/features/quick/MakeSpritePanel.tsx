...
import type { QuickDither, QuickFrameCount, QuickMotion, QuickOutline, QuickPalette, QuickPixelSize, QuickSession } from '../../api'
...
export default function MakeSpritePanel({ session, busy, onSubmit, onBack }: Props) {
  ...
  return <section className="panel quick-make-panel" aria-labelledby="quick-make-heading">
    ...
    <div className="button-row"><button className="primary-button" type="button" disabled={busy} onClick={() => onSubmit({ motion, custom_motion: customMotion, directions, frames, pixelize, pixel_size: pixelSize, palette, dither, outline, background_cleanup: backgroundCleanup, sprite_source: session.source_selection, strategy: 'AUTO' })}>{busy ? text.starting : text.makeSprite}</button><button className="secondary-button" type="button" onClick={onBack}>{text.back}</button></div>
  </section>
}