...
import type { QuickDither, QuickPalette, QuickPixelSize, QuickSession, QuickSourceSelection, QuickPixelizeResult, QuickOutline, QuickPixelMasterStrategy } from '../../api'
...
export default function QuickPixelizePanel({ session, result, busy, onRun, onSelectSource, onBack }: Props) {
  ...
  function run() {
    ...
    setValidation('')
    onRun({ strategy, size: strategy === 'reference_pixel_master_128' ? 128 : size, detail, palette, dither, background, outline, alpha_threshold: alphaThreshold, subject_mode: subjectMode, ...(subjectMode === 'manual' ? { subject_bbox: crop } : {}) })
  }
  ...
  return <section className="panel quick-pixelize-panel" aria-labelledby="quick-pixelize-heading">
    ...
    <p className="muted">{text.pixelizeDescription}</p><fieldset className="choice-group"><legend>Pixel Master strategy</legend><div className="segmented-options" role="group" aria-label="Pixel Master strategy"><button className={strategy === 'preserve' ? 'active' : ''} type="button" aria-pressed={strategy === 'preserve'} onClick={() => setStrategy('preserve')}>Preserve</button><button className={strategy === 'reference_pixel_master_128' ? 'active' : ''} type="button" aria-pressed={strategy === 'reference_pixel_master_128'} onClick={() => { setStrategy('reference_pixel_master_128'); setSize(128) }}>Stylize · C2</button></div>{strategy === 'reference_pixel_master_128' && <span className="helper">Reference-guided AI pixel-style → 128px redraw. Raw, intermediate, and post artifacts are retained.</span>}</fieldset>
    ...
    <fieldset className="choice-group"><legend>{text.spriteSize}</legend><div className="segmented-options" role="group" aria-label={text.spriteSize}>{[64, 96, 128, 192].map((value) => <button className={size === value ? 'active' : ''} type="button" disabled={strategy === 'reference_pixel_master_128' && value !== 128} aria-pressed={size === value} key={value} onClick={() => setSize(value as QuickPixelSize)}>{value}px</button>)}</div><span className="helper">{text.subjectDetected}: {text.spriteSize} means the selected character height.</span></fieldset>
