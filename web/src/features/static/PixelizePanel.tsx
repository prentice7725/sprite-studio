...
import { API_BASE, type QuickAcceptedArtifact, type QuickPixelMasterStrategy } from '../../api'
...
export default function PixelizePanel({ projectId, assetName, disabled = false }: PixelizePanelProps) {
  ...
  const [strategy, setStrategy] = useState<QuickPixelMasterStrategy>('preserve')
  ...
  async function runPixelize() {
    if (strategy === 'preserve' && subjectMode === 'manual' && (crop[2] <= crop[0] || crop[3] <= crop[1])) {
    ...
    try {
      const response = await fetch(`${API_BASE}/static/${encodeURIComponent(projectId)}/pixelize`, {
        ...
        body: JSON.stringify({ strategy, accepted, asset: assetName, size: strategy === 'reference_pixel_master_128' ? 128 : size, detail, subject_mode: subjectMode, ...(strategy === 'preserve' && subjectMode === 'manual' ? { subject_bbox: crop } : {}), palette, dither, background, outline, alpha_threshold: alphaThreshold }),
      })
      if (!response.ok) {
  ...
  return <section className="panel pixelize-panel" aria-labelledby="pixelize-heading">
    <div className="panel-heading"><div><p className="eyebrow">PIXEL MASTER</p><h3 id="pixelize-heading">{text.makePixelMaster}</h3></div><span className="mode-badge">{strategy === 'reference_pixel_master_128' ? 'C2' : 'M1.1'}</span></div>
    <p className="muted">{text.pixelizeDescription}</p>
    <fieldset className="choice-group"><legend>Pixel Master strategy</legend><div className="segmented-options" role="group" aria-label="Pixel Master strategy"><button className={strategy === 'reference_pixel_master_128' ? 'active' : ''} type="button" aria-pressed={strategy === 'reference_pixel_master_128'} onClick={() => { setStrategy('reference_pixel_master_128'); setSize(128) }}>Stylize · C2</button><button className={strategy === 'preserve' ? 'active' : ''} type="button" aria-pressed={strategy === 'preserve'} onClick={() => setStrategy('preserve')}>Preserve</button></div>{strategy === 'reference_pixel_master_128' && <span className="helper">Reference-guided AI redraw at fixed 128px. Raw, intermediate, and post artifacts remain available.</span>}</fieldset>
    ...
    {strategy === 'preserve' && <fieldset className="choice-group"><legend>{text.spriteSize}</legend><div className="segmented-options" role="group" aria-label={text.spriteSize}>{[64, 96, 128, 192].map((value) => <button className={size === value ? 'active' : ''} type="button" aria-pressed={size === value} key={value} onClick={() => setSize(value)}>{value}px</button>)}</div></fieldset>}
    ...
    <details className="advanced-options"><summary>{text.advancedPixelize}</summary><div className="form-row"><label>{text.palette}<select value={palette} onChange={(event) => setPalette(event.target.value === 'auto' ? 'auto' : Number(event.target.value) as PaletteChoice)}><option value="auto">{text.auto}</option>{[16, 24, 32, 48].map((value) => <option key={value} value={value}>{value} {text.colors}</option>)}</select></label>{strategy === 'preserve' && <label>{text.dither}<select value={dither} onChange={(event) => setDither(event.target.value as DitherChoice)}><option value="none">{text.none}</option><option value="ordered-low">Ordered · {text.low}</option><option value="ordered">{text.ordered}</option></select></label>}</div>{strategy === 'preserve' && <div className="form-row"><label>{text.backgroundAlpha}<select value={background} onChange={(event) => setBackground(event.target.value as BackgroundChoice)}><option value="keep">{text.keep}</option><option value="cleanup">{text.cleanup}</option></select></label><label>{text.outline}<select value={outline} onChange={(event) => setOutline(event.target.value as OutlineChoice)}><option value="preserve">{text.preserve}</option><option value="auto">{text.auto}</option></select></label></div>}<label>{text.alphaThreshold}<input type="number" min={1} max={254} value={alphaThreshold} onChange={(event) => setAlphaThreshold(Number(event.target.value))} /></label></details>
