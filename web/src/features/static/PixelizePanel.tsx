...
export default function PixelizePanel({ projectId, assetName, disabled = false }: PixelizePanelProps) {
  ...
  return <section className="panel pixelize-panel" aria-labelledby="pixelize-heading">
    <div className="panel-heading"><div><p className="eyebrow">PIXELIZE M1.1</p><h3 id="pixelize-heading">{text.makePixelMaster}</h3></div><span className="mode-badge">M1.1</span></div>
    ...
    <fieldset className="choice-group"><legend>{text.spriteSize}</legend><div className="segmented-options" role="group" aria-label={text.spriteSize}>{[64, 96, 128, 192].map((value) => <button className={size === value ? 'active' : ''} type="button" aria-pressed={size === value} key={value} onClick={() => setSize(value)}>{value}px</button>)}</div></fieldset>
