...
export type ReviewAction = 'load' | 'analyze' | 'safe' | 'undo' | 'adopt' | 'unadopt' | 'accept' | 'reject'
...
function PipelineAction({ label, state, active, onClick, disabled }: { label: string; state?: string; active: boolean; onClick: () => void; disabled: boolean }) {
  return <button className={`pipeline-action ${active ? 'working' : ''}`} type="button" onClick={onClick} disabled={disabled}><span className={`pipeline-indicator ${state ?? 'not-generated'}`} /> <strong>{label}</strong><small>{active ? 'working…' : state ?? 'ready'}</small></button>
}

function StatusPill({ value }: { value: string }) {
  return <span className={`status-pill ${value.replaceAll(' ', '-')}`}>{value}</span>
}

export function GeneratePanel({ run, state, states, prompt, promptSource, onPromptChange, onSavePrompt, rawAsset, busy, onGenerate, onNormalize, onExtract, status }: { run: RunSummary | null; state: string; states: string[]; prompt: string; promptSource: 'generated' | 'override' | null; onPromptChange: (value: string) => void; onSavePrompt: () => void; rawAsset: string; busy: string; onGenerate: () => void; onNormalize: () => void; onExtract: () => void; status: Record<string, string> }) {
...
export function RefinePanel({ run, state, states, busy, onRefine, previewAsset, summary }: { run: RunSummary | null; state: string; states: string[]; busy: string; onRefine: () => void; previewAsset: string; summary: string }) {
