...

    {
      ...
      "anchors": {                           # optional DIRECTION ANCHOR FRAME PICKS
        "down": {"state": "down_idle",        #   (maintainer 2026-07-25): which curated instance
                 ...
                 "revision": ["a1b2c3d4e5f6"] #   generating its other rows. Any instance of any
                                              ...
                                              #   Resolution/bake SSoT = sprite_studio/curate/anchor.py.
      "recolor": {                           # optional ADOPTED COLOURWAY: which baked
        "picked": "crimson"                  #   recolor variant (sprite_studio/effects/recolor.py,
      ...
      "states": {
        "<state>": {
          ...
          "source_variant": "repaired",      # optional explicit adoption of Studio's
          "source_variant_revision": "...",  # repaired output. Exact repaired bytes and
                                               # their refined source bytes are revision-pinned.
...
- `anchors` missing/direction absent -> that direction's anchor is the anchor row's curated
                                   sequence head (`sprite_studio.curate.anchor.resolve_anchor`).
...
from sprite_studio.spec.runio import load_request

CURATION_FILENAME = "curation.json"
CURATION_KIND = "sprite-studio-curation"
SCHEMA_VERSION = 1
...
def state_breathe(curation: dict[str, Any] | None, state: str) -> dict[str, Any] | None:
    ...
    변형 수학은 봉투 기반 스쿼시&스트레치다 (2026-07-25 교체, `sprite_studio.effects.breathe`).
    ...
    if retired:
        ...
        raise SystemExit(
            ...
            f"{detail}\n"
            f"  마이그레이션: `sprite-studio migrate-breathe <run-dir> --apply` 또는 해당 키를 지우고 "
...
def frame_variant(curation: dict[str, Any] | None, state: str | None = None) -> str:
    """Which frame variant consumers read: ``pixel``, ``plain`` or ``repaired``.

    Resolution order (single source for every consumer):
    1. an explicitly adopted `states.<state>.source_variant == repaired`,
    2. the state's own `states.<state>.pixel_unfake` (the curator's per-row toggle),
    3. the run-wide `pixel_unfake` default (the curator's toggle-all),
    4. absent sidecar / absent fields -> the canonical pixel unfakeed frames.
    ...
    if state is not None:
        entry = curation.get("states", {}).get(state)
        if isinstance(entry, dict) and entry.get("source_variant") == "repaired":
            if not isinstance(entry.get("source_variant_revision"), str):
                raise SystemExit(
                    f"curation state '{state}' selects repaired frames without a source_variant_revision"
                )
            return "repaired"
...
def frame_filename(index: int, variant: str = "pixel") -> str:
    ...
    if variant == "repaired":
        return f"repaired/frame-{index}.png"
...
def _validated_anchor_pins(run_dir: Path, data: dict[str, Any], request: dict[str, Any] | None,
    ...
    `sprite_studio.curate.anchor.resolve_anchor` 가 fail-loud 하고 뷰가 이유를 보여준다 (validator 4차 기각:
...
def load_curation_report(run_dir: Path) -> tuple[dict[str, Any] | None, dict[str, Any]]:
    ...
    if not kind_matches(data.get("kind"), CURATION_KIND):
        raise SystemExit(f"{path} is not a {CURATION_KIND} file")
...
def load_curation(run_dir: Path) -> dict[str, Any] | None:
    ...
    backup semantics."""
    document = load_curation_report(run_dir)[0]
    _require_current_source_variants(run_dir, document)
    return document


def _require_current_source_variants(run_dir: Path, document: dict[str, Any] | None) -> None:
    """Fail loud when an adopted derived variant no longer matches its repair log.

    The curation sidecar stores the exact repaired-output revision that a human
    adopted. Re-running repair creates a new revision and must never silently
    change what compose/export reads under the old approval.
    """
    if not document:
        return
    request = load_request(run_dir)
    for state, entry in (document.get("states") or {}).items():
        if not isinstance(entry, dict) or entry.get("source_variant") != "repaired":
            continue
        expected = entry.get("source_variant_revision")
        repair_dir = run_dir / frames_dir_rel(request, state) / "repair"
        log_path = repair_dir / "repair.log.json"
        try:
            log = json.loads(log_path.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            raise SystemExit(
                f"curation state '{state}' adopts repaired frames, but {log_path} is missing or invalid"
            )
        actual = log.get("output_revision")
        if not isinstance(expected, str) or expected != actual:
            raise SystemExit(
                f"curation state '{state}' adopted repaired revision {expected!r}, "
                f"but the current repair output is {actual!r}; review and adopt it again"
            )
        source_revision = log.get("source_revision")
        source_items = log.get("source_files") or []
        source_paths = [Path(item) if Path(item).is_absolute() else run_dir / item for item in source_items]
        if not isinstance(source_revision, str) or not source_paths or not all(path.is_file() for path in source_paths):
            raise SystemExit(
                f"curation state '{state}' adopts repaired frames, but its refined source set is incomplete"
            )
        source_digest = hashlib.sha256()
        for path in source_paths:
            source_digest.update(path.name.encode("utf-8"))
            source_digest.update(path.read_bytes())
        observed_source = source_digest.hexdigest()[:24]
        if observed_source != source_revision:
            raise SystemExit(
                f"curation state '{state}' repaired source frames changed after repair "
                f"({source_revision} -> {observed_source}); run repair and adopt again"
            )
        outputs = log.get("output_files") or []
        paths = [Path(item) if Path(item).is_absolute() else run_dir / item for item in outputs]
        if not paths or not all(path.is_file() for path in paths):
            raise SystemExit(f"curation state '{state}' adopts repaired frames, but its output set is incomplete")
        digest = hashlib.sha256()
        for path in paths:
            digest.update(path.name.encode("utf-8"))
            digest.update(path.read_bytes())
        observed = digest.hexdigest()[:24]
        if observed != expected:
            raise SystemExit(
                f"curation state '{state}' repaired output bytes changed after adoption "
                f"({expected} -> {observed}); review and adopt again"
            )
...
def state_instances(curation: dict[str, Any] | None, state: str, default_count: int) -> list[int]:
    ...
    검증(sprite_studio.curate.anchor) — 의 SSoT."""
...
def anchor_choices(curation: dict[str, Any] | None) -> dict[str, dict[str, Any]]:
    ...
    인스턴스의 **실재 검증은 해석 시점**(sprite_studio.curate.anchor.resolve_anchor)에 fail-loud."""
...
def write_curation_atomic(run_dir: Path, payload: dict[str, Any]) -> str | None:
    ...
    the webview POST and the `sprite-studio anchor --pick` CLI — stamps and backs up
    ...
    if not kind_matches(payload.get("kind"), CURATION_KIND):
        raise ValueError(f"payload is not a {CURATION_KIND} document")
...
def empty_curation() -> dict[str, Any]:
    return {"version": SCHEMA_VERSION, "kind": CURATION_KIND, "states": {}}
