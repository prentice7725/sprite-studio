...
from sprite_studio.qa import inspect as inspect_mod
from sprite_studio.qa import score as score_mod
...
def _copy_run_dir(source: Path, target: Path) -> None:
    ...
    def ignore(_dir: str, names: list[str]) -> set[str]:
        return {name for name in names if name == ".sprite-studio.lock" or name == "__pycache__"}
    ...
    *,
    ...
    for attempt in range(1, max_passes + 1):
        ...
        if not (next_run_dir / "sprite-request.json").is_file():
            raise SystemExit(f"provider command did not create a sprite-studio run dir: {next_run_dir}")
    ...
    result = {
        ...
        "kind": "sprite-studio-correction-loop-report",
        "dry_run": dry_run,
