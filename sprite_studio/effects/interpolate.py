...
백엔드 = **생성형** (`sprite_studio.gen`: codex `image_gen` / grok Imagine, 기본 codex).
...
from sprite_studio.frames.extract import (extract_component_images, register_row_frames,
                      remove_chroma_background_ycbcr, tighten_components)
from sprite_studio.spec.layout import raw_rel, take_raw_rel
from sprite_studio.spec.runio import load_request, write_request
...
def gen_interpolator(provider: str = "codex") -> Interpolator:
    ...
    def run(img0: Image.Image, img1: Image.Image, t: float, prompt: str) -> Image.Image:
        workdir = Path(tempfile.mkdtemp(prefix="sprite-studio-tween-"))
        try:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.extract:
        from sprite_studio.frames import extract as extract_module
        ...
        print("[interpolate] next: re-extract the FULL batch so the shared palette stays "
              f"consistent -> {sys.executable} -m sprite_studio.frames.extract --run-dir {args.run_dir}")
    return 0
