...
from sprite_studio.gen import PROVIDERS, generate_image
from sprite_studio.spec.layout import guide_rel, prompt_rel, take_raw_rel
...
def record_take(run_dir: Path, state: str, label: str, frames: int) -> None:
    ...
    with publish_guard(run_dir):
        ...
        # 디스크의 키 형태를 그대로 둔다 (스키마 이관은 `sprite-studio migrate-request` 만).
