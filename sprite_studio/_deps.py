...
this module, and `sprite_studio/__init__.py` imports this module, so the gate fires
...
def _missing_numpy_message() -> str:
    ...
    `$SPRITE_STUDIO_ROOT/...` public register so the command is
    copy-pasteable in the shell that just failed; in a canonical deployment the
    two name the same directory.
    """
    return (
        "sprite-studio requires NumPy and this interpreter cannot import it.\n"
        f"  interpreter: {sys.executable}\n"
        f"  sprite-studio:  {REPO_ROOT}\n"
