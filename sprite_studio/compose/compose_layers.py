...
Boundary (SoC): declaration truth is `sprite_studio.compose.layers` (filesystem-free —
...
from sprite_studio.effects.breathe import fit_breathe_pattern, phase_frame, resolve_anatomy
from sprite_studio.compose.compose_atlas import cell_geometry
from sprite_studio.curate.curation import (apply_pixel_edits, apply_transform, edit_index, frame_variant,
                                 load_curation, pixel_snap_scale, source_frame_index,
                                 state_breathe, state_pixel_ops, state_plan, state_revision)
from sprite_studio.frames.extract import heal_run, require_frames_manifest
from sprite_studio.compose.layers import (has_layer_contract, landmark_map, require_valid_layer_request,
                               resolve_stack, state_track)
from sprite_studio.spec.layout import row_frame_rel, state_frame_total
from sprite_studio.spec.runio import acquire_run_dir_lock, atomic_write_set, load_request
...
REPORT_KIND = "sprite-studio-layers-report"
...
def load_report(run_dir: Path | str) -> dict[str, Any] | None:
    ...
    if not kind_matches(report.get("kind"), REPORT_KIND):
        raise SystemExit(f"{path} is not a {REPORT_KIND} ({report.get('kind')!r})")
    return report


# --- CLI -------------------------------------------------------------------
# `sprite-studio compose-layers`, `python -m sprite_studio.compose.compose_layers` and
