...
"""Single sprite-studio CLI entrypoint."""
...
from sprite_studio import gen
from sprite_studio.curate import anchor
from sprite_studio.compose import compose_atlas, compose_cycle, compose_gif, compose_layers, export_aseprite, export_pngs
from sprite_studio.qa import correction_loop, inspect, preview, score
from sprite_studio.frames import cutout, extract, slice_sheet, unpack_atlas
from sprite_studio.gen import prepare
from sprite_studio.gen import normalize_grok_row
from sprite_studio.effects import recolor
from sprite_studio.serve import serve_compose, serve_curation
from sprite_studio.spec import migrate_breathe, migrate_kinds, migrate_request
from sprite_studio.gen.prepare import STYLE_DEFAULT, _outline_config
from sprite_studio.spec.subject import SUBJECTS
...
def _add_normalize_grok_row(p: argparse.ArgumentParser) -> None:
    normalize_grok_row.add_arguments(p)
...
COMMANDS: dict[str, tuple[str, Callable[[argparse.ArgumentParser], None], Callable[..., int]]] = {
    "prepare": ("Prepare a sprite-studio component-row run.", _add_prepare, prepare.run),
    ...
    "preview": ("Build motion-QA previews for a sprite-studio run.", _add_preview, preview.run),
    "compose-cycle": (
    ...
    "migrate-kinds": (
        "Migrate a run's retired sprite-gen-* kind strings to the current sprite-studio-* names.",
        migrate_kinds.add_arguments,
        migrate_kinds.run,
    ),
    "slice-sheet": (
    ...
    "normalize-grok-row": (
        "Normalize a wide Grok multi-subject image into a component-row strip.",
        _add_normalize_grok_row,
    ...
    # webview's own `--help` and this subcommand are the same declaration, so `sprite-studio
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="sprite-studio", description="Unified CLI for the sprite-studio pipeline.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if getattr(args, "_retired_pp", False):
        from sprite_studio.gen.prepare import RETIRED_PP_MESSAGE
