...
"""Serve the sprite-studio curation webview for a single run directory.

Standalone — no server framework, just the standard library plus the dependencies
the pipeline already declares (Pillow, and NumPy via the `sprite_studio` import
below that `heal_run` comes from). Launch it against any sprite-studio run folder
...
PNGs are never touched. The compose scripts read that sidecar and bake the result.
    ...
    sprite-studio curation --run-dir <run-folder>

Three entry forms reach this same module — the console script above, `python -m
sprite_studio.serve.serve_curation`, and the `scripts/serve_curation.py` wrapper. They
...
from sprite_studio.effects.breathe import (DEFAULT_DEPTH, DEFAULT_LAG, freeze_anatomy,
                                reference_key)
from sprite_studio.curate.curation import (CURATION_FILENAME, SCHEMA_VERSION, effective_logical_height,
...
from sprite_studio.frames.extract import heal_run, load_consistent_frames_manifest
from sprite_studio.spec.layout import frames_dir_rel, raw_rel, row_frame_rel, row_orig_rel, state_frame_total
from sprite_studio.spec.runio import load_request, publish_guard, read_guard, write_request
from sprite_studio._modules import qualified
...
def _breathe_source_frame(run_dir: Path, state: str):
    ...
    from sprite_studio.curate.curation import (apply_pixel_edits, apply_transform, edit_index,
...
def _base_grid_response(run_dir: Path, base_path: Path) -> dict:
    ...
    from sprite_studio.frames.extract import (_grid_edges, detect_pixel_grid,
...
def _read_op_progress(run_dir: Path) -> dict | None:
    """엔진이 기록한 진행 파일 (.sprite-studio.progress.json) — 없으면 None."""
    try:
        return json.loads((run_dir / ".sprite-studio.progress.json").read_text(encoding="utf-8"))
...
def _build_run_state_impl(run_dir: Path) -> dict:
    ...
    for state, entry in request["states"].items():
        ...
        for index in range(frame_count):
            ...
            repaired_rel = row_frame_rel(row, index, "repaired") if index < len(row_files) else f"{state_frames_rel}/repaired/frame-{index}.png"
            if (run_dir / repaired_rel).is_file():
                frame["repairedUrl"] = _url(*repaired_rel.split("/"))
                frame["repairedStamp"] = _file_stamp(run_dir / repaired_rel)
...
def _recolor_info(run_dir: Path, curation: dict | None) -> dict | None:
    ...
    was never baked".
    """
    from sprite_studio.effects.recolor import VARIANTS_DIRNAME, load_report
...
def _heal_attempt_key(run_dir: Path) -> tuple:
    ...
    """
    from sprite_studio.frames.extract import engine_revision
...
def _run_module(name: str, run_dir: Path, *extra: str) -> dict:
    ...
    `-m sprite_studio.<step>` resolves through the interpreter's own import path, so it finds
    ...
    installed, so an installed `sprite-studio curation` would 500 on compose/interpolate/reroll/
    export while still serving the SPA. The wrappers are re-exports of these same modules
    (`scripts/compose_sprite_atlas.py` -> `sprite_studio.compose.compose_atlas`), so this is the same
...
def run_interpolate(run_dir: Path, state: str, index_a: int, index_b: int,
    ...
    """AI in-between (`sprite_studio.effects.interpolate`): 테이크 기록 + 전체 배치 재추출.
...
def run_reroll(run_dir: Path, state: str, provider: str = "codex") -> dict:
    """행 리롤 (`sprite_studio.effects.reroll`): 새 테이크 생성 + 전체 배치 재추출.
...
def run_export_gif(run_dir: Path, state: str | None = None, scale: int = 1) -> dict:
    ...
    Reuses `sprite_studio.compose.compose_gif` --run-dir, which applies the same curation
    ...
    def do_GET(self) -> None:
        ...
        if path == "/api/run":
            try:
                ...
                if (self.run_dir / ".sprite-studio.lock").exists():
        ...
        if path == "/api/op-progress":
            ...
            self._send_json({
                "busy": (self.run_dir / ".sprite-studio.lock").exists(),
        ...
        try:
            if path == "/api/curation":
                ...
                with publish_guard(self.run_dir):
                    ...
                    if not stale:
                        ...
                        # 필드는 이월한다.
                        engine_owned = ("frozen", "source_variant", "source_variant_revision")
                        ...
                        # `sprite-studio anchor --pick` 으로 심어둔 지정이 뷰를 모르는 저장
            ...
            if path == "/api/compare-gif":
                ...
                if not isinstance(raw_frames, list) or not raw_frames or len(raw_frames) > 200:
                    ...
                    return
                import base64
                from sprite_studio.util.gif_utils import save_clean_gif
                images = []
                try:
            ...
            if path == "/api/base-edit":
                ...
                if transform_req:
                    ...
                    from sprite_studio.curate.curation import apply_transform as _apply_transform
                    from sprite_studio.curate.curation import normalize_transform as _normalize_transform
...
def add_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    `sprite-studio curation`, `python -m sprite_studio.serve.serve_curation`, and the
...
def run(*, run_dir: Path, host: str = "127.0.0.1", port: int = 0,
    ...
    if not (run_dir / "sprite-request.json").is_file():
        raise SystemExit(f"not a sprite-studio run dir (no sprite-request.json): {run_dir}")
    ...
    print(f"sprite-studio curation webview: {url}")
...
def main(argv: list[str] | None = None) -> int:
    """`python -m sprite_studio.serve.serve_curation` / `scripts/serve_curation.py` entrypoint."""
    # RawDescription: 이 docstring 은 사용 예시를 **들여쓴 별도 줄**로 의도했다.
    # 기본 포매터는 설명 전체를 재줄바꿈해 그 줄을 문단에 녹이는데, 프로그램 이름이
    # 길어지자 예시가 `--run-` / `dir` 로 쪼개져 도움말이 없는 플래그를 광고했다
    # (리네임 회귀, tests/packaging/test_curation_cli_entrypoint.py 가 고정).
