...
to the curation view) is a later slice; this module owns only browse + canvas.
    ...
    sprite-studio compose [--dir <folder>]
...
class ComposeHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        if path == "/api/open-curation":
            ...
            port = _free_port()
            subprocess.Popen(
                [sys.executable, "-m", "sprite_studio.serve.serve_curation",
...
def add_arguments(parser: argparse.ArgumentParser) -> None:
    """Argument surface for `sprite-studio compose`, `python -m sprite_studio.serve.serve_compose`,
...
def run(*, dir: Path | None = None, host: str = "127.0.0.1", port: int = 0,
    ...
    print(f"sprite-studio composition canvas: {url}")
...
def main(argv: list[str] | None = None) -> int:
    """`python -m sprite_studio.serve.serve_compose` entrypoint."""
