...
// 있으면 그것, 없으면 앵커 행의 시퀀스 첫 프레임 (엔진 sprite_studio/anchor.py 와 같은 규칙).
...
function bakeVariant(stateName) {
  const c = run && run.curation && run.curation.states
    ? run.curation.states[stateName] : undefined;
  if (c && c.source_variant === "repaired") return "repaired";
...
function bakeFrame(stateName, frame) {
  if (bakeVariant(stateName) === "repaired") {
    return { url: frame.repairedUrl || null, stamp: frame.repairedStamp || null };
  }
...
function seedEntries() {
  ...
  if (stale.length) {
    ...
    setStatus(`읽을 수 없는 호흡 사이드카: ${names} (${why}) — 원본은 그대로 두었다. `
      + `\`sprite-studio migrate-breathe <run-dir> --apply\` 로 옮기거나 값을 범위 안으로 고쳐라.`, "err");
  }
}
