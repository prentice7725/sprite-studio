...
// 판정은 서버(`sprite_studio/anatomy.py`)가 GET /api/breathe-anatomy 로 한 번 돌려
// 사이드카에 숫자로 얼려두고, 이 파일은 그 숫자로 **워프만** 미러링한다. 검출까지
// JS 가 재구현하면 굽기와 미리보기의 진실이 둘로 갈라진다.
//
// 미러 대상은 sprite_studio/breathe.py 의 fit_breathe_pattern / phase_frame 이다.
// 같은 봉투·같은 정수 연산이라 프리뷰와 굽기가 픽셀 동일해야 한다.

// 서버 상수 미러 (sprite_studio/breathe.py)
