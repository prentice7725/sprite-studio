...
Output (a normal sprite-studio run dir):
...
from sprite_studio.curate.curation import imported_ref_role
from sprite_studio.spec.runio import (
    LOCK_FILENAME,
...
from sprite_studio.frames.segment import mask_components
...
def write_run(
    ...
    source_doc = {
        "version": 1,
        "kind": "sprite-studio-unpack-source",
...
def import_png_groups(out_dir: Path, groups: list[dict[str, Any]], iso: dict[str, Any] | None = None,
    ...
    atomic_write_text(
        ...
        json.dumps({"version": 1, "kind": "sprite-studio-unpack-source", "layout_source": "imported-pngs",
...
def _publish_run(staging: Path, out_dir: Path) -> None:
    ...
    Named isolation strategy: **out_dir (and the `.sprite-studio.lock` inside it) never
