...
from sprite_studio._deps import np
from sprite_studio.curate.curation import effective_logical_height, pixel_snap_scale
from sprite_studio.spec.layout import DERIVED_FRAME_DIRS, frames_dir_rel, raw_rel, take_raw_rel
from sprite_studio.spec.runio import (REQUEST_FILENAME, acquire_run_dir_lock, atomic_save_image,
                              atomic_write_text, load_request, publish_guard, relative_posix,
                              release_run_dir_lock)
from sprite_studio.frames.segment import separate_fused_poses
from sprite_studio.spec.subject import SUBJECT_DEFAULT, default_min_used_pixels, sparse_frame_error, subject_kind
...
# --- public grid seam for Asset Studio refine engines ------------------------
# Studio 의 Sprite/Static Refine Engine 은 같은 격자 신호를 쓰되 한 장이 아니라
# **프레임 집합**(공유 격자) 또는 **큰 장면**(FFT 후보 축소) 위에서 쓴다. 그 조합은
# 여기 있을 것이 아니지만, "엣지를 어떻게 세고 피치를 어떻게 채점하는가" 의 정의는
# 하나여야 한다 — 두 벌이 되면 Studio 가 고른 격자와 엔진이 스냅하는 격자가 갈린다.
# 그래서 정의는 여기 그대로 두고 이름만 공개한다 (동작 변경 없음).
axis_edge_histograms = _edge_histograms
axis_pitch_score = _axis_int_score
axis_pitch_seed = _axis_int_seed
axis_pitch_refine = _axis_refine
...
def _grid_score_edges(rows: tuple[list[list[int]], list[bytes]], w: int, h: int,
    ...
    **정수 정확 산술** — 셀 편차를 float 로 누적하지 않는다. 셀의 한 채널 값 `v` 와
    ...
    `sprite-studio/best-phase-hotspot`): 새 값은 같은 양의 정확판이라 옛 float 누적과
...
def write_pinned_palette(run_dir: Path, palette: list, source: str) -> None:
    atomic_write_text(run_dir / PALETTE_LOCK, json.dumps({
        "kind": "sprite-studio-palette-lock",
        "source": source,
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--normalize-grok-row",
        action="store_true",
        help="normalize each selected wide Grok row in place before extraction",
    )
    ...
    # resolution-aware profile (sprite_studio.spec.subject); the CLI is an
    ...
    if frames_root.is_dir():
        for frame in frames_root.rglob("frame-*.png"):
            if frame.name.endswith(".plain.png") or frame.parent.name == "orig":
                continue
            relative_parent = frame.parent.relative_to(frames_root)
            if any(part in DERIVED_FRAME_DIRS for part in relative_parent.parts):
                continue
...
def _run_locked(args: argparse.Namespace, run_dir: Path):
    ...
    # staging write location. Writer-writer exclusion stays the separate `.sprite-studio.lock`.
    frames_final = run_dir / "frames"
    ...
    if getattr(args, "normalize_grok_row", False):
        # This is an explicit convenience opt-in. Keep ordinary extraction
        # byte-for-byte unchanged and never silently replace a provider raw.
        from sprite_studio.gen.normalize_grok_row import normalize_image

        normalized: list[tuple[Path, Image.Image, dict[str, Any], str]] = []
        for state in states:
            ...
            if not raw_path.is_file():
                continue  # the normal extractor reports the missing raw below
            frame_count = int(request["states"][state]["frames"])
            expected_size = (cell_width * frame_count, cell_height)
            with Image.open(raw_path) as opened:
                if opened.size == expected_size:
                    all_warnings.append(
                        f"{state}: Grok row normalization skipped; raw already matches "
                        f"{expected_size[0]}x{expected_size[1]}"
                    )
                    continue
                try:
                    normalized_image, normalize_report = normalize_image(
                        opened,
                        chroma_key,
                        count=frame_count,
                        ...
                        resample="nearest",
                        align_x=str(fit_config.get("align_x", "foot-centroid")),
                        align_y=str(fit_config.get("align_y", "bottom")),
                    )
                except ValueError as exc:
                    raise SystemExit(f"{state}: Grok row normalization failed: {exc}") from exc
            normalized.append((raw_path, normalized_image, normalize_report, state))

        # Analyze all selected rows before publishing any replacement raw. A bad
        # later row cannot leave earlier rows half-normalized.
        for raw_path, normalized_image, normalize_report, state in normalized:
            atomic_save_image(normalized_image, raw_path)
            report_path = raw_path.with_name(raw_path.stem + ".normalize.report.json")
            normalize_report["input"] = str(raw_path)
            normalize_report["output"] = str(raw_path)
            normalize_report["background"] = "transparent"
            atomic_write_text(report_path, json.dumps(normalize_report, ensure_ascii=False, indent=2) + "\n")
            all_warnings.append(
                f"{state}: Grok row normalized in place to "
                f"{cell_width * int(request['states'][state]['frames'])}x{cell_height}"
            )
    ...
    def _snap_strip(tag: str, strip: Image.Image, frame_count: int) -> dict[str, Any] | None:
        ...
        # sprite-studio/per-frame-pixel-grid): 합의 피치를 프레임에 강제하면 측정차
...
def engine_revision() -> str:
    ...
    import sprite_studio.spec.layout as _layout_module
...
def heal_run(run_dir: Path | str) -> dict[str, Any]:
    ...
    if report["healed"]:
        ...
        for state in candidates:
            cmd = [sys.executable, "-m", "sprite_studio.frames.extract",
...
PROGRESS_REL = ".sprite-studio.progress.json"
