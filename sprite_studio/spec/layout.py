...
# Non-canonical Studio products nested under a state frame directory. The
# extraction manifest owns canonical frame-N.png files only; these trees carry
# their own revision/log contracts and must not be misclassified as orphan
# extraction generations.
DERIVED_FRAME_DIRS = frozenset({"refined", "repair", "repaired"})
...
def row_frame_rel(row: dict[str, Any], index: int, variant: str = "pixel") -> str:
    ...
    if variant == "repaired":
        ...
        return f"{head}/repaired/{name}" if head else f"repaired/{name}"
    return rel
