...
post-process. The general `image-gen` skill is a thin shuttle over `sprite-studio gen`.

CLI:
    sprite-studio gen --provider codex|grok --prompt "..." --out DEST.png
...
def resolve_default_provider() -> tuple[str, dict[str, str] | None]:
    ...
    Precedence: SPRITE_STUDIO_DEFAULT_PROVIDER env > hard default (codex). When the
...
def generate_image(
    ...
    *,
    ...
    workdir = Path(workdir).expanduser().resolve() if workdir else Path(tempfile.mkdtemp(prefix="sprite-studio-gen-"))
...
def _run(args: argparse.Namespace) -> int:
    ...
    if provider is not None:
        ...
        if fallback:
            ...
            print(
                f"sprite-studio gen: default provider '{fallback['from']}' unavailable "
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="sprite-studio gen", description=__doc__)
