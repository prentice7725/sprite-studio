...
from sprite_studio.curate.anchor import anchor_ref_rel
from sprite_studio.frames.extract import color_distance
from sprite_studio.compose.layers import require_valid_layer_request
from sprite_studio.spec.layout import TAXONOMY, guide_rel, prompt_rel, raw_rel
...
def build_generation_plan(request: dict[str, Any]) -> dict[str, Any] | None:
    ...
    # 그 한 장을 고르는 규칙은 `sprite_studio/curate/anchor.py` 가 소유한다 (지정 > 앵커 행 시퀀스
    ...
    stage_rows = [
        {
            ...
            # 기계 소비 필드라 **어디서나 도는 형태**로 굽는다: `sprite-studio` 콘솔 스크립트는
            # 설치된 환경에만 있고(레포 체크아웃엔 없다), 문서의 `sprite-studio anchor` 산문은
            # 레포 관례를 따른다 (같은 커맨드의 두 호출 형태 — 두 진실이 아니다).
            "materialize": (
                f"python3 -m sprite_studio.cli anchor --run-dir . "
    ...
    return {
        "version": 1,
        "kind": "sprite-studio-generation-plan",
        "order": [
...
def _run(args: argparse.Namespace):
    ...
    # cell geometry via sprite_studio.spec.subject.
